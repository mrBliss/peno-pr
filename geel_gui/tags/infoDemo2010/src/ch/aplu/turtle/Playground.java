  ...
  extends JPanel implements Printable
{
  ...
  */
  ...
  /**
   *  Create a Playground inside the given TurtleContainer and with
   *  standard size (400 x 400 pixels) and default background color.
   */
  public Playground() {
    init(new Dimension(402, 402), DEFAULT_BACKGROUND_COLOR);
  }

  protected Playground(boolean isBean) {
    // Only called from TurtlePane
    this.isBean = isBean;
    init(new Dimension(402, 402), beanBkColor);
  }
  ...
  public Playground(Color bkColor) {
    init(new Dimension(402, 402), bkColor);
  }
  ...
  public Playground(Dimension size) {
    init(size, DEFAULT_BACKGROUND_COLOR);
  }

  /**
   * Create a new Playground inside the given TurtleContainer,
   ...
   */
  protected void init(Dimension size, Color bkColor) {
    playgroundSize = size;
    if (isBean)
    {
      turtles = new Vector();
      traceBuffer = new BufferedImage(size.width,
                                      size.height,
